...
create index if not exists idx_doctor_departments_department on public.doctor_departments(department_id, doctor_id);
create unique index if not exists idx_doctor_departments_one_primary on public.doctor_departments(doctor_id) where is_primary;
...
begin
  ...
  update public.doctors set department_id = p_primary_department_id where id = p_doctor_id;
end;
$$;
...
insert into public.doctor_departments (doctor_id, department_id, is_primary)
select id, department_id, true from public.doctors where department_id is not null
on conflict (doctor_id, department_id) do update set is_primary = true;
